...
## Phase 3 での追記: nip.io の IP 誤パース

`pr-1.104.154.128.86.nip.io` が `1.104.154.128` に解決されるバグ級の挙動を踏んだ。

- nip.io は**ダッシュ記法**（`app-104-154-128-86.nip.io`）もサポートしており、
  名前の中から「IP に見える最初の 4 オクテット」を緩く拾う
- `pr-1.104.154.128.86` では `pr-1` の `1` がオクテットとして拾われ
  `1.104.154.128` が返ってしまう（`86` は無視される）
- **プレフィックスがダッシュ + 数字で終わる場合は要注意**

対策: IP の直前に固定ラベルを挟む（採用: `pr-<N>.preview.<IP>.nip.io`）。
切り分けの決め手は `dig +short` で実際の解決結果を見ること。
「Envoy のルートにも endpoint にも問題がないのにタイムアウト」のときは
クラスタ側ではなく名前解決を疑う。
